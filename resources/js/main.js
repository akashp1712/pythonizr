$(function () {
  ...
  function loadMain() {
    ...
    var config = {
      defaultModules: {
        regression: ["dataLoading", "trainTestSplit", "linearReg", "mae"],
        classification: [
          "dataLoading",
          "trainTestSplit",
          "svm",
    ...
    function fillDefaultModules(type) {
      ...
      for (
        var i = 0, curModule;
        (curModule = config.defaultModules[type][i++]);
    ...
    function updateModules() {
      ...
      $("#pre-processing")
        ...
        .each(function () {
          ...
          if (value != "none") {
            ...
            pipelineData["pre-processing"].push(
              dataPreProcessing["data"][value]["name"]
      ...
      $("#post-processing")
        ...
        .each(function () {
          ...
          pipelineData["post-processing"].push(
            dataPostProcessing["data"][value]["name"]
          ); // Add pipeline component
        });
    }
    ...
    function updateCode() {
      ...
      for (var i = 0, code; (code = globalList[i++]); ) {
        codeFull = codeFull.concat(code);
      }
    ...
    function updatePipeline() {
      ...
      for (var i = 0, pipeKey; (pipeKey = keys[i++]); ) {
        ...
        if (pipeData.length > 1) {
          // Pipe has some function (1 is header)
    ...
    if ($("input:checked").length > 0) $("#hidden-section").fadeIn(0);
